...
import { Copy, FolderOpen, Loader2, Trash2 } from 'lucide-react'
...
export function ProjectActions({ projectId, title }: { projectId: string; title: string }) {
  ...
  const [busy, setBusy] = useState<'duplicate' | 'delete' | 'open' | null>(null)
  ...
  const openFolder = async () => {
    ...
    try {
      ...
      setError(`Open folder failed: ${err instanceof Error ? err.message : err}`)
    } finally {
      setBusy(null)
    }
  }
  ...
  return (
    <div className="flex items-center gap-1">
      <Button
        ...
        title="Open project folder (output.mp4 lives here)"
      >
        {busy === 'open' ? <Loader2 className="animate-spin" /> : <FolderOpen />}
      </Button>
      <Button
        type="button"
        variant="ghost"
